...
def get_data(filename, pca):
    ...
    return X_train, y_train, X_test, y_test, look_back, num_features, train_size,look_back, num_features
...
def create_model(look_back, num_features):
    model = Sequential()
    model.add(GRU(32, input_shape=(look_back, num_features), return_sequences=False, kernel_initializer='TruncatedNormal'))
...
def main():
    ...
    png_path = '../result/GRU/' + file + '/available_data.png'
    val_mape_path = '../result/GRU/' + file + "/GRU.best.hdf5"
    val_loss_path = '../result/GRU/' + file + "/GRU.val_loss.hdf5"
    loss_png_path = '../result/GRU/' + file + '/GRU_loss.png'
    history_path = '../result/GRU/' + file + '/GRU_fit_history.csv'
    ffn_path = '../result/GRU/' + file + '/Feedforward result hourly without dummy variables.png'
    path = '../result/GRU/' + file + '/'
    data_path = '../result/LSTM/' + file + '/data.png'

    X_train, y_train, X_test, y_test, look_back, num_features, train_size, look_back, num_features = \
        get_data(filename, pca=True)
    plot_data(png_path)
    ...
    # # Hour
    downsample_results_new(x, y_pred, y_true, magnitude=magnitude, resolution='1H', model_name='GRU', path=path, savefig=True)
    # # Day
    downsample_results_new(x, y_pred, y_true, magnitude=magnitude, resolution='1D', model_name='GRU', path=path, savefig=True)
    # # Week
    downsample_results_new(x, y_pred, y_true, magnitude=magnitude, resolution='1W', model_name='GRU', path=path, savefig=True)
    # # 4 Weeks
    downsample_results_new(x, y_pred, y_true, magnitude=magnitude, resolution='4W', model_name='GRU', path=path, savefig=True)
